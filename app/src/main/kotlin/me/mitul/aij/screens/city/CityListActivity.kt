...
import me.mitul.aij.adapters.CityAdapter
...
class CityListActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val items = dbHelper.getAll(filter)

        val listview = findViewById<RecyclerView>(R.id.recyclerview).apply {
            layoutManager = LinearLayoutManager(applicationContext)
            adapter = CityAdapter(layoutInflater, items, filter)
        }
        ...
        findViewById<EditText>(R.id.ed_search).addTextChangedListener(
            TextFilter(items, object : TextFilter.ListFilter<City> {
                ...
                override fun getText(item: City) = item.city
            })
        )
    }
}
